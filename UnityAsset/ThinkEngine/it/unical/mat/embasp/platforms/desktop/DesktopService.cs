...
namespace EmbASP4Unity.it.unical.mat.embasp.platforms.desktop {
    ...
    using Service = EmbASP4Unity.it.unical.mat.embasp.@base.Service;
    ...
    public Output StartAsync(ICallback callback, IList<InputProgram> programs, IList<OptionDescriptor> options) {
            
            Task<Output> t = Task.Run(() =>
            {
                return StartSync(programs, options);
            });
            t.Wait();
            return t.Result;
        ...
        if (p != null) {
          ...
          foreach (String program_file in p.FilesPaths) {
            ...
            if (File.Exists(program_file) && !((f & FileAttributes.Directory) == FileAttributes.Directory)) {
              ...
              throw new Exception("Warning : the file " + Path.GetFullPath(@program_file) + " does not exists.");
          }
        }
        else
    ...
    protected internal virtual FileStream WriteToFile(string pFilename, string sb) {
      ...
      return tempFile;
        ...
        void Service.StartAsync(ICallback callback, IList<InputProgram> programs, IList<OptionDescriptor> options)
        {
            new Thread(() => {
                callback.Callback(StartSync(programs, options));
            }).Start();
