...
public class SimpleActuator
{
        ...
        foreach(string literal in set.GetAnswerSet())
        {
            ...
            foreach(string mapped in mappedProperties)
            {
                ...
                if (literal.StartsWith(withoutVariable))
                {
                    ...
                    string value = clean.Substring(clean.LastIndexOf('(')+1);
                    ...
                    List<string> property = unityASPVariationNames[clean];
    ...
    {
        unityASPVariationNames = new Dictionary<string, List<string>>();
